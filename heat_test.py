...
def data_generator(df, seq_length=9, batch_size=256):
    while True:
        ...
        for _, row in df.iterrows():
            ...
            if peptide_start == -1:
                continue  # Skip if peptide not found
            peptide_end = peptide_start + len(peptide)

            # Generate samples for:
            # 1. Cleavage site before peptide (peptide_start - 1)
            # 2. All residues in peptide (peptide_start to peptide_end - 1)
            # 3. Cleavage site after peptide (peptide_end)
            for i in [peptide_start - 1] + list(range(peptide_start, peptide_end)) + [peptide_end]:
                if 0 <= i < len(full_seq):
                    # Generate sliding window
                    start = max(0, i - 4)
                    end = min(len(full_seq), i + 5)
                    context_9mer = full_seq[start:end].ljust(seq_length, 'X')
                    context_9mer_one_hot = np.array([one_hot_encode(aa) for aa in context_9mer], dtype=np.float32)

                    # Assign labels
                    label = 1 if (i == peptide_start - 1) or (i == peptide_end) else 0
...
# Load data
df = load_data("specified_trypsin_peptides_with_sequences_6-9.tsv")
